 ...
 * 根据页面名生成页面路径
 */
export const getPagePath = (packageName: string, pageName: string): string => (packageName ? `/${packageName}/pages/${pageName}/${pageName}` : `/pages/${pageName}/${pageName}`)
 ...
 */
export const getParamsString = (params: Record<string, unknown>): string => Object.entries(params).map(([key, value]): string => `${key}=${value}`).join('&')
