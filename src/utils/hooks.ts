...
) => Taro.NodesRef.BoundingClientRectCallbackResult | null | undefined = (ref) => {
  const [info, setInfo] = useState<Taro.NodesRef.BoundingClientRectCallbackResult>()
    ...
    Taro.nextTick(() => {
      if (ref.current) {
        ...
        if (!ref.current.className) {
          throw new Error('[useBoundingClientRect] 传入的Taro.TaroElement对象需要有className')
        }
